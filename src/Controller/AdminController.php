...
/** Administration du catalogue ; l’accès /admin est réservé à ROLE_ADMIN dans security.yaml. */
...
final class AdminController extends AbstractController
{
    ...
    public function newTheme(Request $request, EntityManagerInterface $em): Response
    {
        ...
        if ($name === '') {
            return $this->failure('Le nom du thème est obligatoire.');
        }
        $theme = new Theme($name);
        $theme->setAuditUser($this->getUser()?->getUserIdentifier());
        $em->persist($theme);
        $em->flush();
    ...
    public function editTheme(Theme $theme, Request $request, EntityManagerInterface $em): Response
    {
        ...
        $theme->setName($name);
        $em->flush();
        return $this->success('Thème modifié.');
    }
    ...
    public function deleteTheme(
        ...
        $em->remove($theme);
        $em->flush();
        return $this->success('Thème supprimé.');
    }
    ...
    public function newCurriculum(Request $request, EntityManagerInterface $em): Response
    {
        ...
        if (!$theme || $title === '' || $price === null) {
            return $this->failure('Vérifie le thème, le titre et le prix.');
        }
        $item = new Curriculum($theme, $title, $price);
        $item->setAuditUser($this->getUser()?->getUserIdentifier());
        $em->persist($item);
        $em->flush();
        return $this->success('Parcours créé.');
    }
    ...
    public function deleteCurriculum(
        ...
        $em->remove($item);
        $em->flush();
        return $this->success('Parcours supprimé.');
    }
    ...
    public function newLesson(Request $request, EntityManagerInterface $em): Response
    {
        ...
        if (!$curriculum) {
            return $this->failure('Choisis un parcours pour la leçon.');
        }
        $item = new Lesson($curriculum, 'temp', 1, 0);
        if (!$this->fillLesson($item, $request, $em)) {
            return $this->failure('Vérifie tous les champs de la leçon.');
        }
        $item->setAuditUser($this->getUser()?->getUserIdentifier());
        $em->persist($item);
        $em->flush();
    ...
    public function editLesson(Lesson $item, Request $request, EntityManagerInterface $em): Response
    {
        ...
        $em->flush();
        return $this->success('Leçon modifiée.');
    }
    ...
    public function deleteLesson(
        ...
        $em->remove($item);
        $em->flush();
        return $this->success('Leçon supprimée.');
    }

    /** Partage la validation et l’affectation des champs entre création et modification. */
    private function fillLesson(Lesson $item, Request $request, EntityManagerInterface $em): bool
    {
        ...
        $price = $this->price($request);
        if (!$curriculum || $title === '' || $content === '' || $position < 1 || $price === null) {
            return false;
        }
        ...
        $item->setPriceCents($price);
        $item->setVideoUrl(trim((string) $request->request->get('video_url')));
        return true;
    }

    /** Convertit une saisie en euros en centimes et refuse les valeurs négatives ou non numériques. */
    ...
    /** Vérifie le jeton propre à l’action avant toute modification du catalogue. */
